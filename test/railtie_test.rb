...
class RailtieTest < Minitest::Test
  ...
  def test_config_namespace_exists_on_app
    assert_kind_of ActiveSupport::OrderedOptions, Rails.application.config.otp_rails_resilience
  end
  ...
  def test_breaker_defaults_are_pragmatic_and_boring
    defaults = OtpRails::Resilience::Breaker::DEFAULTS_BY_NAME
  ...
  def test_version_is_dev
    assert_equal "0.1.0.dev", OtpRails::Resilience::VERSION
    assert Gem::Version.new(OtpRails::Resilience::VERSION).prerelease?
  end
end
