...
module Odoshi
  module Resilience
    module Instrumentation
      ...
      # call, so config.odoshi_resilience.instrument_active_record can be
      # flipped without unpatching.
      module ActiveRecord
        module PoolPatch
          def checkout(*args, &block)
            return super unless Odoshi::Resilience.instrument?(:active_record)

            Odoshi::Resilience.breaker(:active_record).call { super(*args, &block) }
          end
        end
